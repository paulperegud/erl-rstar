...
ct: test-build $(CT_CONFIG)
	@mkdir -p logs/
	$(gen_verbose) $(CT_RUN) -suite $(addsuffix _SUITE,$(CT_SUITES)) $(CT_OPTS) $(ERL_CT_OPTS)
endif

define ct_suite_target
ct-$(1): test-build $(CT_CONFIG)
	@mkdir -p logs/
	$(gen_verbose) $(CT_RUN) -suite $(addsuffix _SUITE,$(1)) $(CT_OPTS) $(ERL_CT_OPTS)

fct-$(1): test-build-app $(CT_CONFIG)
	@mkdir -p logs/
	$(gen_verbose) $(CT_RUN) -suite $(addsuffix _SUITE,$(1)) $(CT_OPTS) $(ERL_CT_OPTS)
endef
...
ifneq ($(wildcard src),)
EUNIT_MODULES ?= $(sort $(subst .erl,,$(shell find src -type f -name \*.erl -exec basename {} \;)))
else
EUNIT_MODULES ?=
endif

define eunit_target
funit-$(1): ERLC_OPTS=$(TEST_ERLC_OPTS)
funit-$(1): clean app
	@$(MAKE) --no-print-directory app-build test-dir ERLC_OPTS="$(TEST_ERLC_OPTS)"
	$(gen_verbose) $(ERL) \
	-pa $(TEST_DIR) $(DEPS_DIR)/*/ebin \
	-pz ebin \
	-eval 'case eunit:test([{module, '$(1)'}], [$(EUNIT_OPTS)]) of ok -> halt(0); error -> halt(1) end.'
endef

$(foreach test,$(EUNIT_MODULES),$(eval $(call eunit_target,$(test))))
...
rel_:: distclean-relx-rel relx-rel
endif
